...
impl Plugin for TextInputPlugin {
    fn build(&self, app: &mut bevy::app::App) {
        // Guard `InputDispatchPlugin` so we don't panic when another
        // plugin (e.g. one of the experimental `bevy_ui_widgets`
        // headless widgets, or `bevy_feathers`) has already added it.
